...
func testTransactionToOneUserBankUsingFromBank(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.FromBankID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.FromBank().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadFromBank(ctx, tx, false, (*[]*Transaction)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.FromBank == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.FromBank = nil
	if err = local.L.LoadFromBank(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.FromBank == nil {
		t.Error("struct should have been eager loaded")
...
func testTransactionToOneUserBankUsingToBank(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.ToBankID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ToBank().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TransactionSlice{&local}
	if err = local.L.LoadToBank(ctx, tx, false, (*[]*Transaction)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ToBank == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ToBank = nil
	if err = local.L.LoadToBank(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ToBank == nil {
		t.Error("struct should have been eager loaded")
...
func testTransactionToOneSetOpUserBankUsingFromBank(t *testing.T) {
	var err error
	...
	for i, x := range []*UserBank{&b, &c} {
		err = a.SetFromBank(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.FromBankTransactions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.FromBankID, x.ID) {
			t.Error("foreign key was wrong value", a.FromBankID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.FromBankID))
		reflect.Indirect(reflect.ValueOf(&a.FromBankID)).Set(zero)
		...
		if !queries.Equal(a.FromBankID, x.ID) {
			t.Error("foreign key was wrong value", a.FromBankID, x.ID)
		}
	}
}

func testTransactionToOneRemoveOpUserBankUsingFromBank(t *testing.T) {
	var err error
	...
	if err = a.SetFromBank(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveFromBank(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.FromBank().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.FromBankID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.FromBankTransactions) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}

func testTransactionToOneSetOpUserBankUsingToBank(t *testing.T) {
	var err error
	...
	for i, x := range []*UserBank{&b, &c} {
		...
		if x.R.ToBankTransactions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.ToBankID, x.ID) {
			t.Error("foreign key was wrong value", a.ToBankID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ToBankID))
		reflect.Indirect(reflect.ValueOf(&a.ToBankID)).Set(zero)
		...
		if !queries.Equal(a.ToBankID, x.ID) {
			t.Error("foreign key was wrong value", a.ToBankID, x.ID)
		}
	}
}

func testTransactionToOneRemoveOpUserBankUsingToBank(t *testing.T) {
	var err error
	...
	if err = a.SetToBank(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveToBank(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.ToBank().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.ToBankID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.ToBankTransactions) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}
...
var (
	transactionDBTypes = map[string]string{`ID`: `uuid`, `UserID`: `uuid`, `FromBankID`: `uuid`, `ToBankID`: `uuid`, `ExpensesCategoryID`: `uuid`, `TransferFee`: `numeric`, `Amount`: `numeric`, `Descriptions`: `text`}
	_                  = bytes.MinRead
)
